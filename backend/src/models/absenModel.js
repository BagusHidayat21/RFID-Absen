...
const AbsenModel = {
  insertAbsen: async (siswa_id, status) => {
    const query = `
  ...
  getAllAbsen: async (siswa_id) => {
    const query = "SELECT * FROM absen";

    try {
      const result = await pool.query(query);
      return result.rows;
    } catch (err) {
      console.error("❌ Error getting absen by siswa:", err);
      throw err;
    }
  },

  getAbsenById: async (siswa_id) => {
