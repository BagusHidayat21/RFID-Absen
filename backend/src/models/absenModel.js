...
const AbsenModel = {
  ...
  getAllAbsen: async () => {
    try {
      const result = await pool.query("SELECT siswa.id AS siswa_id, siswa.nama, siswa.nis, siswa.kelas_id, absensi.tanggal, absensi.jam, absensi.status, absensi.keterangan FROM absensi JOIN siswa ON absensi.siswa_id = siswa.id ORDER BY absensi.created_at DESC;");
      return result.rows;
    } catch (err) {
      throw err;
    }
