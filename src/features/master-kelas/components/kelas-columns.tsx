// Column definitions for Master Kelas table with DropdownMenu actions
'use client';
...
interface GetKelasColumnsProps {
  onDetail: (item: KelasRowData) => void;
  onEdit: (item: KelasItem) => void;
  onDelete: (item: KelasRowData) => void;
}

export function getKelasColumns({
  onDetail,
  onEdit,
  onDelete,
}: GetKelasColumnsProps): ColumnDef<KelasRowData>[] {
  return [
    {
      accessorKey: 'id',
      header: ({ column }) => (
    ...
    },
    {
      accessorKey: 'nama',
      header: ({ column }) => (
        <DataTableColumnHeader column={column} title="NAMA TINGKAT" />
      ),
      cell: ({ row }) => (
        <span className="font-semibold text-slate-900">Kelas {row.original.nama}</span>
      ),
    },
    {
