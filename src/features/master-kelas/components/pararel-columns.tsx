// Column definitions for Master Rombel Pararel table with DropdownMenu actions
'use client';
...
interface GetPararelColumnsProps {
  onDetail: (item: PararelRowData) => void;
  onEdit: (item: PararelItem) => void;
  onDelete: (item: PararelRowData) => void;
}

export function getPararelColumns({
  onDetail,
  onEdit,
  onDelete,
}: GetPararelColumnsProps): ColumnDef<PararelRowData>[] {
  return [
    {
      accessorKey: 'id',
      header: ({ column }) => (
    ...
    },
    {
      accessorKey: 'nama',
      header: ({ column }) => (
        <DataTableColumnHeader column={column} title="NAMA ROMBEL" />
      ),
      cell: ({ row }) => (
        <span className="font-semibold text-slate-900">Rombel {row.original.nama}</span>
      ),
    },
    {
      accessorKey: 'studentCount',
      header: ({ column }) => (
        <DataTableColumnHeader column={column} title="SISWA TERDAFTAR" />
      ),
      cell: ({ row }) => (
